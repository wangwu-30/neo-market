"use strict";
/**
 * Production deployment script for agent-market on Sepolia (L1).
 *
 * Required env:
...
async function main() {
    ...
    console.log("");
    // Check balance
    const balance = await hardhat_2.ethers.provider.getBalance(deployer.address);
    console.log(`Deployer balance: ${hardhat_2.ethers.formatEther(balance)} ETH`);
    if (balance === 0n) {
        throw new Error("Deployer balance is 0. Please fund the deployer address.");
    }
    // 1. ModuleRegistry
